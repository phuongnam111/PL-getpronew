...
async def api_genqr(response):
    ...
    if 'generate-qr' in response.url:
        ...
        try:
            ...
            logger.info(f"API response body (JSON): {response_body}")
        except Exception as e:
            response_body = await response.text()  # Fallback to text if JSON fails
            logger.info(f"API response body (Text): {response_body}")

async def login_sale_dashboard(page):
    """Logs into the FNB sale dashboard."""
    try:
        logger.info("FNB - Wallet - Momo Generate QR code: %s", ProdConfig.fnbnshop)
...
async def generate_qr_code(page):
    """Checks if the FNB Momo QR code is visible."""
    try:
        await page.click(UiSelectors.fnb_pay)
        await page.click(UiSelectors.FNB_RAD_QR_MM)
        await page.click(UiSelectors.FNB_MM_SHOWQR)
        qr_visible = await page.is_visible(UiSelectors.FNB_MM_QR_CONTENT, timeout=UiSelectors.timeout)
        if qr_visible:
            logger.info("FNB - Momo QR is visible.")
        else:
            logger.warning("FNB - Momo QR is not visible.")
        return qr_visible
    except Exception as e:
        logger.error(f"Failed during FNB QR check: {e}")
        await snapshot(page, "fnb-MomogenQR-error.png")
        return False

async def verify_qr(page):
    """Verifies if the QR code is visible."""
    try:
        qr_visible = await page.is_visible(UiSelectors.FNB_MM_QR_CONTENT)
        if qr_visible:
