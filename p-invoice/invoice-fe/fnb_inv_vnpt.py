...
async def login_to_system(page):
    try:
        logger.info("FNB invoice setting template partner = VNPT: %s", ProdConfig.fnbnshop)
        ...
        await snapshot(page, "fnb-vnpt-login-error.png")
...
async def navigate_to_einvoice_settings(page):
    try:
        ...
        await snapshot(page, "fnb-vnpt-setting-tab-error.png")

async def select_and_save_vnpt_template(page):
    try:
        await dismiss_popup_if_present(page, UiSelectors.XP_BTN_SKIPP)
        #CONFIG ACCOUNT INVOICE; PARTNER =vnpt
        await page.wait_for_selector(UiSelectors.FNB_PARTNER_VNPT, state="visible", timeout=10000)
        await page.click(UiSelectors.FNB_PARTNER_VNPT)
        await page.click(UiSelectors.XP_INV_FB_CONTINUE)
        #account fill
        await page.fill(UiSelectors.TXT_LINKLOGIN_VNPT, ProdConfig.VNPT_LINK_LOGIN)
        await page.fill(UiSelectors.TXT_TAX_VNPT, ProdConfig.VNPT_MST)
        await page.fill(UiSelectors.TXT_ACC_VNPT, ProdConfig.VNPT_ACC)
        await page.fill(UiSelectors.TXT_PASS_VNPT, ProdConfig.VNPT_PASS)
        await page.fill(UiSelectors.TXT_LINKSEARCH_VNPT, ProdConfig.VNPT_LINK_LOGIN)
        ...
        await snapshot(page, "fnb-vnpt-select-template-error.png")
...
async def publish_invoice(page):
    ...
    try:
        ...
        await page.get_by_role("option", name=ProdConfig.VNPT_TEMPLATE).click()
        ...
        await snapshot(page, "fnb-vnpt-publish-error.png")
        raise
