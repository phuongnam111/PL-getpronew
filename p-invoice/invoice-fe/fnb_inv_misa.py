...
async def login_to_system(page):
    try:
        logger.info("FNB invoice setting template partner = Misa: %s", ProdConfig.fnbnshop)
        ...
        await snapshot(page, "fnb-misa-login-error.png")

#navigate_to_einvoice_settings
async def navigate_to_einvoice_settings(page):
    try:
        ...
        await snapshot(page, "fnb-misa-setting-tab-error.png")

async def select_and_save_misa_template(page):
    try:
        await dismiss_popup_if_present(page, UiSelectors.XP_BTN_SKIPP)
        #CONFIG ACCOUNT INVOICE; PARTNER =MISA
        await page.wait_for_selector(UiSelectors.FNB_PARTNER_MISA, state="visible", timeout=10000)
        await page.click(UiSelectors.FNB_PARTNER_MISA)
        await page.click(UiSelectors.XP_INV_FB_CONTINUE)
        #account fill
        await page.fill(UiSelectors.TXT_TAX_MISA, ProdConfig.MISA_MST)
        await page.fill(UiSelectors.TXT_ACC_MISA, ProdConfig.MISA_ACC)
        await page.fill(UiSelectors.TXT_PASS_MISA, ProdConfig.MISA_PASS)
        ...
        await snapshot(page, "fnb-misa-select-template-error.png")

async def verify_einvoice_setting_status(page):
    try:
        if await page.is_visible(UiSelectors.XP_TXT_COMPLETESET):
            logger.info("FNB - MISA - E-Invoice Setting Template Successfully Updated")
            return True
        else:
            logger.error("FNB - MISA Failed to update the E-Invoice Setting Template")
            return False
    except Exception as e:
...
async def publish_invoice(page):
    ...
    try:
        ...
        await page.get_by_role("option", name=ProdConfig.MISA_TEMPLATE).click()
        ...
        await snapshot(page, "fnb-misa-publish-error.png")
        raise
...
async def main():
    ...
    try:
        async with async_playwright() as p:
            ...
            await select_and_save_misa_template(page)
