...
async def login_to_system(page):
    try:
        logger.info("Retail invoice setting template partner = MISA: %s", ProdConfig.retailname2)
...
async def navigate_to_einvoice_settings(page):
    try:
        ...
        await page.click(UiSelectors.XP_RD_MISA)
...
async def verify_einvoice_setting_status(page):
    try:
        if await page.is_visible(UiSelectors.TXT_SUCCESS_MS):
            logger.info("RETAIL - MISA - E-Invoice Setting Template Successfully Updated")
            return True
        else:
            logger.warning("RETAIL - MISA Failed to update the E-Invoice Setting Template")
            return False
    except Exception as e:
...
async def publish_invoice(page):
    ...
    try:
        ...
        await page.click(UiSelectors.MS_RT_TEM_PUBLISH)
        ...
        await snapshot(page, "retail-misa-publish-error.png")
        return False
