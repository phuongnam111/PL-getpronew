...
async def login_to_system(page):
    try:
        logger.info("Retail invoice setting template partner = Viettel: %s", ProdConfig.retailname2)
...
async def navigate_to_einvoice_settings(page):
    try:
        ...
        await page.click(UiSelectors.XP_RD_VIETTEL)
...
async def select_and_save_viettel_template(page):
    try:
        ...
        # if not  await page.locator(UiSelectors.TXT_SUCCESS_VT).wait_for(state='visible', timeout=UiSelectors.timeout):
        #     raise Exception("No success message")
        # return True
    except Exception as e:
...
async def verify_einvoice_setting_status(page):
    try:
        if await page.is_visible(UiSelectors.TXT_SUCCESS_VT):
            logger.info("RETAIL - VIETTEL - E-Invoice Setting Template Successfully Updated")
            return True
        else:
            logger.warning("RETAIL - VIETTEL Failed to update the E-Invoice Setting Template")
            return False
    except Exception as e:
        logger.error(f"Failed to verify e-invoice setting status: {e}")
        return False

# Publish invoice partner = VIETTEL
async def publish_invoice(page):
    ...
    try:
        ...
        """)
        logger.info("Navigated to sale dashboard and publish invoice = VIETTEL")
        ...
        await page.click(UiSelectors.VT_RT_TEM_PUBLISH)
        await page.keyboard.press('F9')
        logger.info("Invoice published without print dialog")
        
    except Exception as e:
        logger.error(f"Failed to publish invoice: {e}")
        await snapshot(page, "retail-viettel-publish-error.png")
        return False
