...
async def login_to_dashboard(page):
    """Logs into the retail dashboard and navigates to the manager screen."""
    try:
        logger.info("Retail - payment-hub onboarding: %s", ProdConfig.retailname2)
        ...
        logger.info("Login successful, navigating to dashboard screen.")
    except Exception as e:
        logger.error(f"Failed to log in to dashboard: {e}")
        raise
...
async def payment_screen(page):
    try:
        ...
        await snapshot(page, "retail-paymenthub-error.png")
        raise
